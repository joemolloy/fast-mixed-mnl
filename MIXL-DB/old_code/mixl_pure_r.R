...
data[ , mackie := inc_imp / mean(inc_imp)  ]
data$mackie[data$mackie==0] <- 0.0001 # numerical issues
hist(data$mackie)
summary(data$mackie)

data[ , distie := distance / mean(distance) ]
data$distie[data$distie==0] <- 0.0001 # numerical issues
data$distie[data$distie>15] <- 13 # numerical issues
hist(data$distie)
summary(data$distie)

hist(data$daily_mean_air_temp1[data$daily_mean_air_temp1 < 99999])
hist(data$precipitation_0700_1900[data$precipitation_0700_1900 < 99999])
hist(data$sunshine_duration[data$sunshine_duration < 99999])

data[ , cold := (data$daily_mean_air_temp1 <= 7)  ]
data[ , hot := (data$daily_mean_air_temp1 > 15)  ]
data[ , rainy := (data$precipitation_0700_1900 > 0.2)  ]
data[ , sunny := (data$sunshine_duration > 200)  ]
...
colnames(draws)=c(paste0("d",rep(1:dimensions)))

# turn into data table and add indices for people and draws

draws=data.table(draws)
draws[,ID:=rep(unique(data$ID),each=Ndraws)]
draws[,draws_index:=rep(1:Ndraws,N)]
draws[,merge_index:=ID+draws_index/100000]

# Expand datatable to replicate each row R times, where R = number of draws

data=data[rep(1:nrow(data),each=Ndraws),]

# add a column with index for draws

data[,draws_index:=rep(1:Ndraws,nrow(data)/Ndraws)]

# add an index for merging

data[,merge_index:=ID+draws_index/100000]

# add actual draws to data table by merging data and draws

setkey(data,merge_index)
setkey(draws,merge_index)

# merge draws into main data.table
...
# parallelization: create a data list, containing the data frames by split-id's as entries
# split data into number of cores, by ID

if (parallelcomputing==1) {
  setDT(data)[ , id := .GRP, by = ID]
  splitid <- sample(1:kernel,N,rep=TRUE)
  id <- c(1:N)
  splitter <- data.frame(id,splitid)
  data <- merge(data, splitter, by.x = "id", by.y = "id", all = TRUE)
  
  if (mixing==1) {
    data_list <- by( as.data.frame(data), data$splitid, function(subdat) {
      subdat <- as.data.table(subdat); setkey(subdat, merge_index); subdat } ) } 
  else { data_list <- by( as.data.frame(data), data$splitid, function(subdat) {
      subdat <- as.data.table(subdat); setkey(subdat, rowid); subdat } ) }
} #%parallelcomputing==1%
